...
- `POST /reset`
  Returns: `{"observation": <Observation>}`
- `POST /step` 
  Expects Body: `{"direction": "<up|down|left|right>"}`
  Returns: `{"observation": <Observation>, "reward": <float>, "done": <bool>}`
- `GET /state`
  Returns: `<Observation>`
...
2. **Start the OpenEnv FastAPI Server:**
   ...
   ```
3. **Run the Streamlit UI (in a new terminal window):**
   ```bash
